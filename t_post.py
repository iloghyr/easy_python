...
articleId = '676347775'
comment = '支持支持'
refer = 'http://family.baxidu.com/core/index.jsp?chc=2110383119'
louceng = 800
...
def doPost(articleId = '', refer = '' , comment = ''):
    try:
        global cookie
        r = urllib2.Request('http://family.bxaidu.com/comment/comment/addComment.do')
        r.add_header('Host','family.baxidu.com')
...
def doCheck(articalId='',refer=''):
    try:
        global cookie
        r = urllib2.Request('http://family.bxaidu.com/comment/comment/load.do?1=1')
        r.add_header('Host','family.baxidu.com')
        r.add_header('Cookie',cookie)
        r.add_header('Referer',refer)
        r.add_header('User-Agent','Mozilla/5.0 (Windows NT 6.1; rv:19.0) Gecko/20100101 Firefox/19.0')
